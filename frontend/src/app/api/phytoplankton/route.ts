...
export async function POST(req: Request) {
  try {
    ...
    // Keep as-is but coerce counts to numbers where possible
    const data = body
      .map((item: Record<string, unknown>) => ({
        phytoplanktonscientificName: String(
          item?.phytoplanktonscientificName ?? item?.scientificName ?? item?.name ?? ""
        ),
        "no of that pyhtoplankon": isNaN(Number(item?.["no of that pyhtoplankon"]))
          ? String(item?.["no of that pyhtoplankon"] ?? item?.count ?? "0")
          : Number(item?.["no of that pyhtoplankon"]) ?? Number(item?.count) ?? 0,
      }))
